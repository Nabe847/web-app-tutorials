...
> 💡 **ヒント**:  
> Webアプリケーションでは、ユーザーからの入力を受け付けて処理することが重要です。  
...
2. **Todo追加の仕組み**
   ...
   - `tasks.length > 0 ? ... : 1`: Todoが1つもない場合は1から始めます
   - `setTasks([...tasks, { id: newId, title: newTask, completed: false }])`: 新しいTodoを配列に追加します
   - `setNewTask('')`: 入力フォームを空にします

3. **フォームの基本**
   - `onSubmit`: フォームの送信ボタンが押されたときの処理を指定します
   - `value`: 入力フォームに表示する文字を指定します
   - `onChange`: 入力フォームの文字が変わったときの処理を指定します
   - `placeholder`: 入力フォームが空のときに表示する説明文を指定します
