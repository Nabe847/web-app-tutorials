...
このチュートリアルでは、Next.js を使ってタスク管理アプリを作成しながら、Web アプリケーション開発の基本を学んでいきます。
...
10. **app フォルダ**
    ...
    - このフォルダ内のファイル名が、ブラウザでアクセスする URL になります
    - 例：`page.js` → `http://localhost:3000/` で表示されます

> 💡 **ヒント**:
>
> - 最初は `src/app/page.js` と `src/app/globals.css` を編集するだけで、簡単な変更ができます
...
1. **Next.js プロジェクトを作成するコマンドは？**
   ...
   **答え: B: `npx create-next-app`**
   ...
   - このコマンドを実行することで、新しい Next.js プロジェクトを簡単に作成できます
   </details>
