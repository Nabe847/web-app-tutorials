...
前のステップに続いて、今回はAPIのエラー処理を改善します。より分かりやすいエラーメッセージを提供し、APIの使いやすさを向上させましょう。

1. **カスタム例外クラスの作成**
2. **エラー処理の実装**
3. **動作確認**

> 💡 **エラー処理**: APIで発生する可能性のあるエラーを適切に処理し、ユーザーに分かりやすいメッセージを返すことです。
...
APIをテストして、エラー処理が正しく動作するか確認しましょう：
...
<summary>答えを見る</summary>
   ...
   **答え: A: `class CustomError(HTTPException):`**
   ...
   - `HTTPException`を継承することで、FastAPIのエラー処理機能を利用できます
